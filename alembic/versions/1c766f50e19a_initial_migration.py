...
def upgrade() -> None:
    ...
    data integrity at the business logic level because the API data often includes
    references before the referenced objects. For example:
    ...
    The application handles data integrity by:
    ...
    op.create_table(
        "attachments",
        sa.Column("id", sa.Integer(), nullable=False),
        sa.Column("postId", sa.BigInteger(), nullable=True),
...
def downgrade() -> None:
    ...
    # PostgreSQL: Foreign keys are handled by the database automatically during DROP operations
    # No PRAGMA equivalent needed
