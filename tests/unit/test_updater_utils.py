...
def test_perform_update_valid(monkeypatch):
    ...
    monkeypatch.setattr(utils, "print_warning", lambda msg: None)
    monkeypatch.setattr(utils, "print_info", lambda msg: None)
    result = utils.perform_update(program_version, release_info)
...
def test_check_for_update_no_release_info(monkeypatch):
    ...
    config = DummyConfig()
    monkeypatch.setattr(utils, "get_release_info_from_github", lambda version: None)
...
def test_check_for_update_draft(monkeypatch):
    ...
    monkeypatch.setattr(
        utils, "get_release_info_from_github", lambda version: release_info
    )
    result = utils.check_for_update(config)
    assert result is False
...
def test_check_for_update_outdated(monkeypatch):
    ...
    monkeypatch.setattr(utils, "perform_update", lambda current, new_release: False)
    result = utils.check_for_update(config)
    assert result is False
