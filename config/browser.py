            ...
            for table in tables:
                ...
                for row in rows:
                    if row[0] == "session_active_session":
                        session_active_session = json.loads(row[5].decode("utf-8"))[
                            "token"
                        ]
                        break
...
def get_browser_config_paths() -> list[str]:
    ...
    if platform.system() == "Windows":
        ...
        if appdata is None or local_appdata is None:
            raise RuntimeError(
                "Windows OS AppData environment variables are empty but shouldn't."
            )
