...
def dedupe_init(config: FanslyConfig, state: DownloadState):
    ...
    if state.download_path and state.download_path.is_dir():
        print_info(
            f"Deduplication is automatically enabled for:\n{17 * ' '}{state.download_path}"
        )
...
def dedupe_media_file(
    config: FanslyConfig, state: DownloadState, mimetype: str, filename: Path
) -> bool:
    ...
    if "image" in mimetype:
        ...
        if "audio" in mimetype:
            ...
            raise RuntimeError(
                "Internal error during media deduplication - invalid MIME type passed."
            )
    ...
    if file_hash in hashlist:
        if config.show_downloads and config.show_skipped_downloads:
            print_info(
                f"Deduplication [Hashing]: {mimetype.split('/')[-2]} '{filename.name}' → skipped"
            )
        filename.unlink()
