...
### Category K: FakeSpy (MockInternal in spy disguise) — 12 violations
...
### Category L: Fixture issues — 11 shadowing + 3 scope bugs
...
- `test_engine` (session) + `test_database_sync` (session) → parallel isolation breaks
- `timeline_data`, `conversation_data` (session) → mutable dicts, state leakage risk
...
### Wave 4: Integration test fixture cleanup + shallow-test consolidation (est. 12–20 hours)
