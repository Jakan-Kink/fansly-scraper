...
async def download_single_post(config: FanslyConfig, state: DownloadState):
    ...
    print_info("You have launched in Single Post download mode.")

    if config.post_id is not None:
        print_info(
            f"Trying to download post {config.post_id} as specified on the command-line ..."
        )
        ...
        if post_object["accountMediaBundles"] or post_object["accountMedia"]:
            ...
            if creator_username is None:
                ...
                if creator_display_name and creator_username:
                    print_info(
                        f"Inspecting post {post_id} by {creator_display_name} (@{creator_username})"
                    )
                else:
                    print_info(
