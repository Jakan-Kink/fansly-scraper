...
def display_release_notes(program_version: str, release_notes: str) -> None:
    ...
    """
    print_update(
        f"Successfully updated to version {program_version}!\n\n► Release Notes:\n{release_notes}"
    )
    print()
...
def parse_release_notes(release_info: dict) -> str | None:
    ...
    body_match = re.search(r"```(.*)```", release_body, re.DOTALL | re.MULTILINE)
...
def perform_update(program_version: str, release_info: dict) -> bool:
    ...
    """
    print_warning(
        "A new version of Fansly Downloader NG has been found on GitHub - update recommended."
    )
    ...
    print_warning(
        "To update Fansly Downloader NG, please download the latest version from the GitHub repository."
    )
