...
class OptimizedSQLiteMemory:
    ...
    def __init__(self, db_path: str | Path, config: FanslyConfig):
        ...
        Raises:
            sqlite3.DatabaseError: If database is corrupted and can't be accessed
        """
        ...
        # Create a temporary connection to check database integrity
        temp_conn = None
        try:
            temp_conn = sqlite3.connect(self.local_path)
            if not self._check_database_integrity(temp_conn):
                print_error("Database is corrupted")
                print_error(
                    "Please restore from backup or delete the database to start fresh"
                )
    ...
    def _convert_to_utf8(self, connection: sqlite3.Connection) -> None:
        ...
        try:
            ...
            try:
                ...
                print_error("Database appears to be corrupted")
                print_error(f"Please restore from backup: {e}")
                raise
    ...
    def _check_database_integrity(self, connection: sqlite3.Connection) -> bool:
        ...
        Raises:
            sqlite3.DatabaseError: If database is inaccessible
        """
        cursor = None
        try:
            cursor = connection.cursor()
            # Run quick_check first as it's faster
            cursor.execute("PRAGMA quick_check")
    ...
    async def close(self) -> None:
        """Close all connections and sync to remote location asynchronously."""
        with self.thread_lock:
    ...
    def close_sync(self) -> None:
        """Synchronous version of close for cleanup."""
        # Try to acquire lock with timeout
        if not self.thread_lock.acquire(timeout=5):  # 5 second timeout
            print_warning("Could not acquire thread lock for cleanup, forcing close")
            return
        ...
        try:
            ...
            if hasattr(self, "conn"):
                try:
                    ...
                    print_error(f"Error closing main connection: {e}")
            ...
            if hasattr(self, "sync_manager"):
                try:
                    ...
                    print_error(f"Error during sync manager cleanup: {e}")
        finally:
...
# Add new asyncio-specific connection tracking
_async_connections: dict[int, tuple[Any, int]] = {}
_async_connections_lock = asyncio.Lock()


@event.listens_for(Engine, "engine_disposed")
def receive_engine_disposed(engine):
    """Handle engine disposal by cleaning up thread-local connections."""
    ...
    # Also clean up any async connections for this engine
    if hasattr(engine, "_async_engine"):
        task_id = id(asyncio.current_task()) if asyncio.current_task() else None
        if task_id and task_id in _async_connections:
            conn, _ = _async_connections[task_id]
            del _async_connections[task_id]


@event.listens_for(Engine, "close")
def receive_close(dbapi_connection, connection_record):
    """Ensure DBAPI connections are properly closed."""
...
class Database:
    ...
    def _setup_engines_and_sessions(self) -> None:
        ...
        # Store reference to async engine in sync engine for cleanup
        self.sync_engine._async_engine = self.async_engine
    ...
    def _setup_event_listeners(self) -> None:
        ...
        def on_checkin(dbapi_conn, connection_record):
            """Ensure connections are checked in properly."""
            try:
                cursor = dbapi_conn.cursor()
                cursor.execute("SELECT 1")  # Test connection
                cursor.close()
            except Exception:
                # If connection test fails, invalidate it
                connection_record.invalidate()

        @event.listens_for(self.sync_engine, "engine_connect")
        def on_connect(connection):
            """Register thread cleanup on connection."""
    ...
    def get_sync_session(self) -> Generator[Session]:
        ...
        # No existing session or invalid session, create a new one
        # Check if optimized connection exists
        if not hasattr(self, "_optimized_connection"):
            print_warning("No optimized connection available, using direct session")
        ...
        # Try to acquire lock with timeout
        if not self._optimized_connection.thread_lock.acquire(
            timeout=5
        ):  # 5 second timeout
            print_warning(
                "Could not acquire thread lock for session creation, using direct session"
            )
            # Create session without lock
            with self.sync_session() as session:
                try:
                    yield session
                finally:
        ...
        try:
            ...
            # Check if optimized connection still exists before releasing lock
            if hasattr(self, "_optimized_connection"):
                self._optimized_connection.thread_lock.release()
    ...
    async def get_async_session(self) -> AsyncGenerator[AsyncSession]:
        ...
        async with _async_connections_lock:
            if task_id in _async_connections:
                session, ref_count = _async_connections[task_id]
                # Verify session is still valid
                try:
                    ...
                    _async_connections[task_id] = (session, ref_count + 1)
                    try:
                        yield session
                    finally:
                        # Decrement reference count
                        if task_id in _async_connections:
                            _, ref_count = _async_connections[task_id]
                            if ref_count <= 1:
                                await session.close()
                                del _async_connections[task_id]
                            else:
        ...
        async with self._optimized_connection._get_thread_async_lock():
            async with self.async_session() as session:
                _async_connections[task_id] = (session, 1)
                try:
                    yield session
                finally:
                    if task_id in _async_connections:
    ...
    def _close_all_connections(self) -> None:
        ...
        for thread_id in dir(_thread_local):
        ...
        # Dispose engines to close all connections
    ...
    def _run_sync_cleanup(self) -> None:
        """Run synchronous cleanup operations without any async operations."""
        try:
            # Close all thread-local sessions immediately
            for thread_id in dir(_thread_local):
                if thread_id == "session":
                    session_info = getattr(_thread_local, "session")
                    session, _ = session_info
            ...
            # Force dispose engines
            if hasattr(self, "async_engine"):
                # SQLAlchemy 2.0 style - dispose the underlying sync engine
                self.async_engine.sync_engine.dispose()
            if hasattr(self, "sync_engine"):
                self.sync_engine.dispose()

            # Force close all async connections
            if "_async_connections" in globals():
                for task_id in list(_async_connections.keys()):
                    conn, _ = _async_connections[task_id]
                    try:
                        # Force close without awaiting
                        conn.close()
                    except Exception:
                        pass
                    _async_connections.pop(task_id, None)

            # Close optimized connection if it exists
            if hasattr(self, "_optimized_connection"):
                self._optimized_connection.close_sync()
        except Exception as e:
    ...
    def close(self) -> None:
        ...
        try:
            # Always use sync cleanup to avoid event loop issues
    ...
    async def close_async(self) -> None:
        ...
        try:
            print_info("Starting async database cleanup")
            # Use a timeout to prevent hanging
            async with asyncio.timeout(10):
                # First cleanup async connections and sessions
                print_info("Cleaning up async connections")
                await self._cleanup_async_connections()

                # Then dispose engines
                print_info("Disposing database engines")
                if hasattr(self, "async_engine"):
                    await self.async_engine.dispose()
                    delattr(self, "async_engine")
                if hasattr(self, "sync_engine"):
                    self.sync_engine.dispose()
                    delattr(self, "sync_engine")

                # Finally close optimized connection synchronously
                print_info("Closing optimized connection")
                if hasattr(self, "_optimized_connection"):
                    self._optimized_connection.close_sync()
                    delattr(self, "_optimized_connection")

                print_info("Database cleanup completed successfully")
        except (TimeoutError, Exception) as e:
            print_error(f"Error during async database cleanup: {e}")
            print_info("Attempting sync cleanup as fallback")
            # Fallback to sync cleanup on timeout/error
            self._run_sync_cleanup()

    async def _cleanup_async_connections(self) -> None:
        """Cleanup async connections properly."""
        try:
            # Close any active async sessions
            if hasattr(self, "_async_session_factory"):
                print_info("Found async session factory, cleaning up sessions")
                # Get all active sessions
                active_sessions = [
                    session
                    for session in self._async_session_factory.registry._connections
                ]
                print_info(f"Found {len(active_sessions)} active sessions")

                # Close each session with individual timeouts
                for session in active_sessions:
                    try:
                        async with asyncio.timeout(2):  # Timeout per session
                            if session.is_active:
                                print_info(f"Closing active session {id(session)}")
                                await session.close()
                            else:
                                print_info(f"Skipping inactive session {id(session)}")
                    except TimeoutError:
                        print_error(f"Timeout closing session {id(session)}")
                    except Exception as e:
                        print_error(f"Error closing session {id(session)}: {e}")

                print_info("Session cleanup completed")
        except Exception as e:
            print_error(f"Error cleaning up async connections: {e}")
...
def require_database_config(func: Callable[..., RT]) -> Callable[..., RT]:
    ...
    Returns:
        Wrapped function that ensures database config is available
...
def get_database_config(args: tuple[Any, ...], kwargs: dict[str, Any]) -> Any:
    """Get database configuration from args/kwargs.
    ...
    Returns:
        Config object with database attribute
...
def has_session(kwargs: dict[str, Any]) -> bool:
    """Check if a session is already provided in kwargs.

    Args:
        kwargs: Function keyword arguments

    Returns:
        True if a session is provided and not None
    """
    return "session" in kwargs and kwargs["session"] is not None


def validate_session_type(func: Callable[..., RT], async_session: bool) -> None:
    """Validate that function and session types match.

    Args:
        func: Function to validate
        async_session: Whether async session is requested

    Raises:
        ValueError: If function and session types don't match
    """
    is_async = asyncio.iscoroutinefunction(func)
...
def with_database_session(
    ...
    2. If not, creates a new session using config._database
    3. Ensures proper session cleanup after function execution

    Args:
        async_session: Whether to use async session (default: False)
    ...
    def decorator(func: Callable[..., RT]) -> Callable[..., RT]:
        validate_session_type(func, async_session)

        @wraps(func)
        async def async_wrapper(*args: Any, **kwargs: Any) -> RT:
            if has_session(kwargs):
                return await func(*args, **kwargs)

            config = get_database_config(args, kwargs)
